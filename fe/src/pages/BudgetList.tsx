...
export default function BudgetList() {
  ...
  const getActualSpent = (catId: string) => {
    return transactions
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold light:text-gray-800 dark:text-white">Ngân sách</h2>
      ...
      <div className="space-y-4">
        {budgets.map((b) => {
          ...
          const actualSpent = getActualSpent(b.categoryId);
            ...
            <div key={b.id} className="light:bg-white dark:bg-gray-900 rounded-lg shadow p-5">
              <div className="flex justify-between items-center mb-2">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-sm font-medium light:text-gray-600 dark:text-gray-400">{actualSpent.toLocaleString('vi-VN')} / {b.limitAmount.toLocaleString('vi-VN')} VNĐ</span>
                  <button onClick={() => startEdit(b)} className="text-indigo-600 hover:text-indigo-800"><Pencil size={16} /></button>
